...
namespace eClothesAPI.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public IActionResult GetOrderByUserId(int userid)
        {
            try
            {
                var order = _repository.Order.GetOrderByUserId(userid);
                if (order == null)
                {
                    _logger.LogError($"Order with userid: {userid}, hasn't been found in db.");
                    return NotFound();
                }
                else
                {
                    _logger.LogInfo($"Returned order with details for userid: {userid}");
                   
                    var orderResult = _mapper.Map<List<OrderListDTO>>(order);
